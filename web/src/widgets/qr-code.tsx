...
function QRCodeWidget() {
  ...
  const { output, isPending } = useToolInfo<"qr-code">();
  ...
  return (
    <div className={`${theme} bg-neutral-primary`}>
      <div className="bg-neutral-primary-soft block max-w-sm p-6 border border-default rounded-base shadow-xs">
        <div className="rounded-base bg-brand flex justify-center items-center p-4">
          <QRCodeSVG
            value={output.link}
            size={224}
            fgColor="#ffffff"
        ...
        <h5 className="mt-6 mb-2 text-2xl font-semibold tracking-tight text-heading text-center">
          QR Code
        </h5>
        <p className="text-body text-center">
          Use your phone's camera to scan the QR code and instantly open the link.
        </p>
      </div>
    </div>
  );
